...
def valorOptimo(etapa, estado):
    if etapa == n + 1:
        # Si la etapa actual es la ultima, el valor es 0 si estado (numero de productos por obtener) = 0 y 200 si el estado == 1
        if estado == 0:
            return 0
        else:
            return 200
        ...
        aux = [(K(0) + 0 + ((1/3)**0)*valorOptimo(etapa + 1, 1), etapa, 0,estado)]
        x = 1
        while True:
            aux.append((K(x) + (5*x) + ((1/3)**x)*valorOptimo(etapa + 1, 1), etapa, x,estado))
...
print(f"costo de la politica optima = {pOptima} ")
print()
